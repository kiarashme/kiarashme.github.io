...
TW.showKeyboardCallbacks = function() {
    ...
    var i, list = state.keyboard_callbacks_docs,
        len = list.length;
...
TW.createBarnSolidColor = function(width, height, depth, color) {
    ...
    var barnMaterial = new THREE.MeshBasicMaterial({
        color: colorHex
    });
...
TW.stringifyGeometry = function(geom) {
    ...
    for (f = 0; f < numFaces; f++) {
        var face = faces[f];
        result += "face " + f + " w/normal: " + JSON.stringify(face.normal) + " and materialIndex: " + face.materialIndex + "\n";
